App.Views.BeneficiaryEditor = App.Helpers.RowEditor.extend({
  ...
  TEMPLATE: " \
    <tr> \
      ...
      <td> \
        <input name='relationship' class='relationship' type='text' style='border: 1px solid #efefef; width: 70px' /> \
      </td> \
      <td><input name='telephone' class='telephone' type='text' style='border: 1px solid #efefef; width: 100px' /></td> \
  ...
  deleteItem: function(id, callback) {
    $(this.el).find(".errors").hide();
    if (!id) callback();
    else if (confirm("Etes-vous sur de vouloir supprimer ce bénéficiaires?")) {
      $.ajax({
        type: "DELETE",
        url: this.url + id,
        success: callback
